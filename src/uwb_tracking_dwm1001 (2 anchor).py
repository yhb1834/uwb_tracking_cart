...
def Control_motor(linear_vel, angular_vel, target_x, target_y):
    ...
    angular_vel_cmd = angular_vel * target_angle_diff
...
def calculate_point_coordinates2(distance1, distance2):

    if distance1 == 0:
        x = point1[0]
        y = point1[1]
    else:
        global cos_a, cos_past
        global x_past, y_past, turn
        ...
        if distance1-distance2 > 0:
            ...
            cos_a = (distance2*distance2 + dist_anchor*dist_anchor - distance1*distance1) / (2*distance2*dist_anchor)
            x = -(distance2 * cos_a - abs(point2[0]))
        '''
        ...
        if turn == 0:
            ...
            if abs(x_past - x) > 2:
                x = (sum(x_list)/len(x_list)) * 0.7 + x_past * 0.3
            if abs(y_past - y) > 2:
                y = (sum(y_list)/len(y_list)) * 0.7 + y_past * 0.3
            x_past = x
            y_past = y
            turn += 1
        ...
        try:
            while not rospy.is_shutdown():
                ...
                try:
                    ...
                    #tag_x, tag_y = calculate_point_coordinates(anchor1_result, anchor2_result, anchor3_result)
                    tag_x, tag_y = calculate_point_coordinates2(anchor1_result, anchor2_result)
                    anchor1_2_dist = 0.5 # m
                    ...
                    if distance < min_dist :
                       ...
                       linear_vel_cmd, angular_vel_cmd = Control_motor(0.1, 0.2, tag_x, tag_y)
                       cmd_vel_publisher.publish_cmd_vel(linear_vel_cmd, angular_vel_cmd)

                    elif move_range <= distance < max_dist:
                       linear_vel_cmd, angular_vel_cmd = Control_motor(0.3, 0.5, tag_x, tag_y)
                       cmd_vel_publisher.publish_cmd_vel(linear_vel_cmd, angular_vel_cmd)
                    
                    #print("cmd_vel : ", linear_vel_cmd, angular_vel_cmd)
                    self.pub_cos_a.publish(cos_a)
